...
Three adjacent conflations fell with the same pass. A response that arrived but will not decode as JSON is now `malformed_feed` rather than `unreachable` — parsing is split from fetching, so a 502 HTML body no longer reads as a transport outage. A non-string `location` used to escape the per-entry guard (it is never `.strip()`ed) and abort the whole run inside `is_virtual()`'s `.lower()`; it is now a counted malformed record on both sources. And the "web search fallback needed" warning is suppressed under `feed_failure`, so a format outage cannot present itself to callers as a normal empty result. `references/run-state.md` records the widened `fetch` checkpoint artifact.

Both per-entry guards narrow from `except Exception` to an explicit `RECORD_SHAPE_ERRORS` tuple, so a programming defect in the parser propagates instead of being laundered into a "the feed drifted" count — the counter is only as honest as what it agrees to catch. The run-state checkpoint manifest moves to `schema_version` 2, because a saved `fetch` artifact now carries `sources` and `feed_failure`. `begin` migrates it the way `stateful-artifacts` prescribes — the owner detects the older version, upgrades the record, and rewrites it — rather than wiping the store: the run keeps its `run_date`, the `fetch` stage and everything downstream of it drop (truncation, not filtering, since resume means "start at the first stage not in `completed`"), and a stage the shape change never touched survives. An unrecognized version is still not upgradeable and resets. `references/run-state.md` carries the per-version steps.

A JSON `null` `cfpLink` now reads as the same "no CFP open" case a missing one does. Stripping it blind raised, counted the record malformed, and let a healthy feed of link-less conferences escalate all the way to `feed_failure` — a false technical failure the health work would otherwise have introduced; a truthy non-string is still malformed. The decode boundary now catches the whole family of input-driven decoder failures — `ValueError` (which covers `JSONDecodeError` and the digit-limit error a 5,000-digit integer literal raises) plus `RecursionError`, which is not a `ValueError`. Naming only `JSONDecodeError` let the other two escape and abort the run, taking the healthy source's results with them.

Two more classification edges went the safe way. `untilDate: 0` is a numeric timestamp (1970) and so a closed CFP: deciding on presence and type rather than truthiness keeps it in `records_filtered`, where it cannot escalate an otherwise healthy source to `feed_failure`. And the run-state manifest's `schema_version` is validated as a real JSON integer before migration — `True == 1` and `2.0 == 2` in Python, so a bare equality test let `true` enter the v1 upgrade path and `2.0` pass as current, both bypassing the reset a malformed manifest should get.

Two wordings stopped overclaiming. The no-results warning reads "No usable CFPs from the primary sources" rather than "Both primary sources returned empty" — one source can be down while the other is validly empty, and a source that never answered did not return an empty feed. And the wrapper's technical-failure condition points at `feed_failure` instead of "fetch or format failures", which read as though two partially usable feeds should fail the run.
