...
A JSON `null` `cfpLink` now reads as the same "no CFP open" case a missing one does. Stripping it blind raised, counted the record malformed, and let a healthy feed of link-less conferences escalate all the way to `feed_failure` — a false technical failure the health work would otherwise have introduced; a truthy non-string is still malformed. `RecursionError` joins `JSONDecodeError` on the decode path, since a document nested past the interpreter's limit is the decoder failing on input and should produce a health record rather than an escaping traceback.
