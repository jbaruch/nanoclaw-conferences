...
- **Source inference** — stored entries with no `source` field infer it from the `cfp_url` host: `sessionize.com` → `sessionize-speaker-api`; `developers.events` → `developers.events`; `javaconferences.org` → `javaconferences.org`; else unsourced (non-Sessionize branch). The inferred value is written back in Step 8.
