...
- **Run failure** — Step 1 (check-cfps) hits a technical failure: every primary feed reporting a fetch or format failure, or a required tool denied. Step 2 is skipped intentionally. The cursor stays at its prior value, so the next eligible cycle's precheck either keeps gating (if still inside the cadence window) or wakes the agent for a retry (if the window has elapsed).
