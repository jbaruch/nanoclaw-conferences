...
- **Run failure** — Step 1 (check-cfps) hits a technical failure: its fetch script reporting `feed_failure == true`, or a required tool denied. The flag's predicate is the script's contract (`skills/check-cfps/scripts/check-cfps-fetch.py` docstring). Step 2 is skipped intentionally. The cursor stays at its prior value, so the next eligible cycle's precheck either keeps gating (if still inside the cadence window) or wakes the agent for a retry (if the window has elapsed).
