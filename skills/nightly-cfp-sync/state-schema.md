...
- **Run failure** — Step 1 (check-cfps) hits a technical failure: its fetch script raising `feed_failure` (every primary feed delivering nothing usable for a technical reason — the flag's predicate is `scripts/check-cfps-fetch.py`'s contract), or a required tool denied. A partially usable feed is not a failure. Step 2 is skipped intentionally. The cursor stays at its prior value, so the next eligible cycle's precheck either keeps gating (if still inside the cadence window) or wakes the agent for a retry (if the window has elapsed).
